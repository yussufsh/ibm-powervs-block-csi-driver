...
func runRemoteCommand(publicIP string, arg ...string) (string, error) {
	args := []string{"-i", sshDefaultKey, "-o", "StrictHostKeyChecking=no", "-o", "UserKnownHostsFile=/dev/null", publicIP, "--"}
...
func runCommand(cmd string, args ...string) (string, error) {
	...
	if err != nil {
		return string(output), fmt.Errorf("command [ssh %s] failed with error: %v", strings.Join(args, " "), err.Error())
	}
...
func geIBMPISession() (*ibmpisession.IBMPISession, error) {
	var err error
	...
	if piID, err = getEnvVar("POWERVS_CLOUD_INSTANCE_ID"); err != nil {
		return nil, err
	}
